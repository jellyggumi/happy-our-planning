...
대한민국 지도 기반 **행사 검색 · 신청 매크로 · 결과 알람 · AI 추천 플래닝** 경량 인프라.
무료 공공 API + flat-file 지식 DB(llm-wiki) + 무료 SaaS(free tier)로 월 0원 운영을 목표로 한다.
...
flowchart TD
  ...
  C --> D["run_pipeline<br/>수집→정규화→OKF적재→검증→인덱스→wiki"]
  D --> E["unittest · 22종"]
  E --> F{변경 diff?}
...
flowchart TB
  ...
  OKF -.-> UPS & BI & WI
  RP --> REC & NOT & MAC
  BUILD --> WEB["web/public/data/*.json → index.html"]
```
...
scripts/      ingest · normalize · build · macro · notify · recommend
...
python -m scripts.run_pipeline          # 수집→정규화→OKF적재→검증→인덱스→wiki
python -m unittest discover -s tests    # 단위 테스트 22종
python -m scripts.recommend.rank        # 규칙기반 AI 추천 + 주간 플랜(JSON)
